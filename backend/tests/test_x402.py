"""Tests for the x402-gated /acquire-research endpoint (x402 v2).
...
class TestPaymentRequired:
    def test_no_payment_header_returns_402_challenge(self, client):
        response = client.get("/acquire-research")

        assert response.status_code == 402
        body = response.json()
        assert body["x402Version"] == 2
        assert "error" not in body
        assert body["resource"]["url"].endswith("/acquire-research")
        (req,) = body["accepts"]
    ...
    def test_malformed_payment_header_returns_402(self, client):
        response = client.get("/acquire-research", headers={"PAYMENT-SIGNATURE": "not-base64!!!"})
    ...
    def test_non_object_payment_payload_returns_402(self, client):
        ...
        response = client.get("/acquire-research", headers={"PAYMENT-SIGNATURE": header})
...
class TestPaidRequest:
    def test_valid_payment_unlocks_content(self, client, facilitator_ok):
        response = client.get("/acquire-research", headers={"PAYMENT-SIGNATURE": VALID_HEADER})
    ...
    def test_v1_x_payment_header_is_also_accepted(self, client, facilitator_ok):
        response = client.get("/acquire-research", headers={"X-PAYMENT": VALID_HEADER})
    ...
    def test_facilitator_receives_decoded_payment_and_requirements(self, client, facilitator_ok):
        ...
        client.get("/acquire-research", headers={"PAYMENT-SIGNATURE": VALID_HEADER})
